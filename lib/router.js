...
Router.route('/admin/order/:_id', {
	name: 'adminOrder',
	// access to order all data
	waitOn: function(){
		var order = Meteor.subscribe('adminOrder', this.params._id);
		var photos = Meteor.subscribe('adminOrderPhotos', this.params._id);
		return [order, photos];
	},
	// data (this) is current orders
	data: function(){
